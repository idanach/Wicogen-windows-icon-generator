    ...
    def bulk_mode(file):
        ...
        if file_extension.lower() in supported_file_types:
            ...
            if os.path.exists(dst_folder_path):
                ...
                generate_icon(f'{source_path}/{file}', f'{dst_folder_path}/icon.ico',
            ...
            if os.path.exists(dst_folder_path):
                write_desktop_ini_file(dst_folder_path)
                copy_file(f'{source_path}/{file}', f'{dst_folder}/{file_name}/icon.ico', overwrite_files)
    ...
    def single_mode():
        ...
        if file_extension.lower() in supported_file_types:
            if os.path.exists(dst_folder):
                ...
                generate_icon(source_path, f'{dst_folder}/icon.ico', keep_image_dimensions, folder_style,
                              overwrite_files)
        elif file_extension.lower() == '.ico':
            if os.path.exists(dst_folder):
                write_desktop_ini_file(dst_folder)
                copy_file(source_path, f'{dst_folder}/icon.ico', overwrite_files)
        elif os.path.isdir(f'{source_path}/{file}'):
            print(f"{Fore.LIGHTYELLOW_EX}[Error] Code_9: Unsupported file type. (folder){Style.RESET_ALL}")
        else:
            print(f'{Fore.LIGHTRED_EX}[Error] Code_10: Unsupported file type.{Style.RESET_ALL}{file}')
